    ...
    <header className="z-10 border-b border-ink-200 bg-white/90 backdrop-blur dark:border-ink-800 dark:bg-ink-950/80">
      <div className="mx-auto flex w-full max-w-6xl items-center gap-3 px-4 py-3">
        ...
        <div className="min-w-0">
          ...
          <div className="truncate text-xs text-ink-500">{scopeLabel}</div>
        </div>

        <nav className="ml-3 flex items-center gap-1">
          <NavLink href="/dashboard" current={active === "chat"}>Copilot</NavLink>
          <NavLink href="/painel" current={active === "board"}>Meu Painel</NavLink>
        </nav>
    ...
    <Link
      ...
      className={`rounded-lg px-3 py-1.5 text-sm font-medium transition ${
        current
          ...
          : "text-ink-500 hover:bg-ink-100 hover:text-ink-800 dark:hover:bg-ink-800 dark:hover:text-ink-200"
      }`}
    >
      {children}
    </Link>
